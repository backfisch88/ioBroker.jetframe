...
async function enforceCacheLimit(adapter, dir, maxFiles, logDebug) {
  try {
    const files = await adapter.readDirAsync("jetframe.admin", dir);
    const entries = (files || []).filter((f) => (f == null ? void 0 : f.file) && !f.isDir);
    if (entries.length <= maxFiles) {
      return;
    }
    entries.sort((a, b) => {
      var _a, _b;
      const ta = new Date(a.modifiedAt || ((_a = a.stats) == null ? void 0 : _a.mtime) || 0).getTime();
      const tb = new Date(b.modifiedAt || ((_b = b.stats) == null ? void 0 : _b.mtime) || 0).getTime();
      return ta - tb;
    });
