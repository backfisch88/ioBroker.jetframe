...
async function fetchAdsb(config, httpJsonRaw, logWarn) {
	...
	for (const url of urls) {
		...
		const arr = Array.isArray(body == null ? void 0 : body.aircraft)
			? body.aircraft
			: Array.isArray(body == null ? void 0 : body.ac)
				? body.ac
				: [];
...
function buildAdsbUrls(config) {
	...
	const airportLat = Number(
		(_c = (_b = (_a = anyConfig.airport) == null ? void 0 : _a.lat) != null ? _b : anyConfig.airportLat) != null
			? _c
			: config.homeLat,
	);
	const airportLon = Number(
		(_f = (_e = (_d = anyConfig.airport) == null ? void 0 : _d.lon) != null ? _e : anyConfig.airportLon) != null
			? _f
			: config.homeLon,
	);
