...
__export(states_exports, {
  ...
  trackDirectionText: () => trackDirectionText,
...
async function ensureBaseStates(adapter, config) {
  ...
  try {
    ...
    if ((speechEnabledState == null ? void 0 : speechEnabledState.val) === null || (speechEnabledState == null ? void 0 : speechEnabledState.val) === void 0) {
...
function trackDirectionText(degRaw, lang = "en") {
  const deg = Number(degRaw);
  if (!Number.isFinite(deg)) {
    return "";
  }
  const normalized = (deg % 360 + 360) % 360;
  const dirs = import_lang.COMPASS_DIRECTIONS[lang] || import_lang.COMPASS_DIRECTIONS.en;
...
async function setForeignStateChanged(adapter, id, value, ack = true) {
  try {
...
async function writeFlight(adapter, config, base, a) {
  const routeCallsign = String(a.routeCallsign || a.callsign || "").trim().toUpperCase();
  ...
  const rawCallsign = String(a.rawCallsign || a.callsign || "").trim();
  const cleanedCallsign = String(a.cleanedCallsign || a.callsign || "").trim().replace(/\s+/g, "").toUpperCase();
  ...
  await setForeignStateChanged(adapter, `${base}.probableRunway`, a.probableRunway || "", true);
  await setForeignStateChanged(adapter, `${base}.probableRunwayText`, a.probableRunwayText || "", true);
  await setForeignStateChanged(adapter, `${base}.probableRunwayHeading`, a.probableRunwayHeading || 0, true);
  await setForeignStateChanged(adapter, `${base}.probableRunwayDiffDeg`, a.probableRunwayDiffDeg || 0, true);
  await setForeignStateChanged(adapter, `${base}.runwayConfidence`, a.runwayConfidence || 0, true);
  ...
  await setForeignStateChanged(adapter, `${base}.distAirportNm`, (0, import_geo.round)(a.distAirportNm || 0, 1), true);
  await setForeignStateChanged(adapter, `${base}.bearingHomeDeg`, (0, import_geo.round)(a.bearingHomeDeg || 0, 1), true);
  await setForeignStateChanged(adapter, `${base}.windowDiffDeg`, (0, import_geo.round)(a.windowDiffDeg || 0, 1), true);
  await setForeignStateChanged(
    adapter,
    `${base}.bearingAircraftToAirportDeg`,
    (0, import_geo.round)(a.bearingAircraftToAirportDeg || 0, 1),
    true
  );
  await setForeignStateChanged(
    adapter,
    `${base}.bearingAirportToAircraftDeg`,
    (0, import_geo.round)(a.bearingAirportToAircraftDeg || 0, 1),
    true
  );
  await setForeignStateChanged(adapter, `${base}.landingTrackDiffDeg`, (0, import_geo.round)(a.landingTrackDiffDeg || 0, 1), true);
  await setForeignStateChanged(adapter, `${base}.takeoffTrackDiffDeg`, (0, import_geo.round)(a.takeoffTrackDiffDeg || 0, 1), true);
  await setForeignStateChanged(adapter, `${base}.airportTrackDiffDeg`, (0, import_geo.round)(a.airportTrackDiffDeg || 0, 1), true);
...
async function buildDisplayInfo(adapter, config, a) {
  var _a, _b, _c;
  ...
  const configuredAirportLabel = airportLabel(
    ((_a = config.airport) == null ? void 0 : _a.name) || ((_b = config.airport) == null ? void 0 : _b.iata) || "",
    ((_c = config.airport) == null ? void 0 : _c.iata) || ""
  );
  const mode = String(a.mode || "").toUpperCase();
  const departureAirport = airportLabel(originDisplayName, a.originIata) || (mode === "TAKEOFF" ? configuredAirportLabel : "");
  const approachAirport = airportLabel(destDisplayName, a.destIata) || (mode === "LANDING" ? configuredAirportLabel : "");
...
function airportLabel(name, iata) {
...
function modeVisTextFromFlight(a, originDisplayName, destDisplayName, lang) {
  const mode = String(a.mode || "").toUpperCase();
  if (mode === "TAKEOFF") {
    return `\u{1F6EB} ${(0, import_lang.t)(lang, "takeoff")} ${originDisplayName !== "\u2014" ? originDisplayName : ""}`.trim();
  }
  if (mode === "LANDING") {
    return `\u{1F6EC} ${(0, import_lang.t)(lang, "landing")} ${destDisplayName !== "\u2014" ? destDisplayName : ""}`.trim();
  }
  if (mode === "OVERFLIGHT") {
    return `\u{1F6E9}\uFE0F ${(0, import_lang.t)(lang, "overflight")}`;
  }
  return `\u2708\uFE0F ${(0, import_lang.t)(lang, "flight")}`;
}
function windowPositionInfo(a, lang) {
  const bearing = Number(a.bearingHomeDeg || 0);
  const diff = Number(a.windowDiffDeg || 0);
  if (!bearing && !diff) {
    return {
      text: `\u2197\uFE0F ${(0, import_lang.t)(lang, "positionUnknown")}`,
      className: "side",
      speechText: (0, import_lang.t)(lang, "atWindow")
    };
  }
  const abs = Math.abs(diff);
  if (abs <= 8) {
    return {
      text: `\u2B06\uFE0F ${(0, import_lang.t)(lang, "directlyInFrontOfWindow")}`,
      className: "center",
      speechText: (0, import_lang.t)(lang, "directlyInFrontOfWindow")
    };
  }
  if (diff < 0) {
    return {
      text: `\u2B05\uFE0F ${(0, import_lang.t)(lang, "leftOfWindow")} \xB7 ${Math.round(abs)}\xB0`,
      className: "side",
      speechText: (0, import_lang.t)(lang, "leftOfWindow")
    };
  }
  return {
    text: `\u27A1\uFE0F ${(0, import_lang.t)(lang, "rightOfWindow")} \xB7 ${Math.round(abs)}\xB0`,
    className: "side",
    speechText: (0, import_lang.t)(lang, "rightOfWindow")
  };
}
async function aircraftDisplayInfo(adapter, config, a) {
  ...
  let manufacturer = (0, import_lang.t)(config.contentLang, "manufacturerFallback");
  ...
  if (manufacturerLogoUrl && config.cacheExternalImages) {
    manufacturerLogoUrl = await (0, import_images.cacheExternalLogoUrl)(
      adapter,
      manufacturerLogoUrl,
      manufacturer,
      import_images.MANUFACTURER_LOGO_CACHE_DIR,
...
function findConfiguredManufacturerLogo(config, manufacturer, allText) {
...
function parseManufacturerLogoConfig(raw) {
  ...
  }).filter((entry) => !!(entry == null ? void 0 : entry.key) && /^https?:\/\//i.test(entry.url));
}
function aircraftSizeLabel(allText) {
  const all = String(allText || "").toUpperCase().replace(/[\s_-]/g, "");
  ...
  if (/A300|A310|A330|A332|A333|A338|A339/.test(all) || /A340|A342|A343|A345|A346/.test(all) || /A350|A359|A35K|A351/.test(all) || /B757|B752|B753/.test(all) || /B767|B762|B763|B764/.test(all) || /B777|B772|B773|B77W|B778|B779|B77L|B77F/.test(all) || /B787|B788|B789|B78X/.test(all) || /DC10|MD11|IL86|IL96|L1011/.test(all)) {
    return "Widebody";
  }
  if (/A300F|A330F|B763F|B752F|B733F|B734F|B738F|AN12|AN26/.test(all)) {
    return "Cargo";
  }
  if (/A220|A221|A223|BCS1|BCS3|CS100|CS300/.test(all) || /A318|A319|A320|A321|A20N|A21N/.test(all) || /B707|B717|B727|B737|B731|B732|B733|B734|B735|B736|B738|B739|B38M|B39M/.test(all) || /DC8|DC9|MD80|MD81|MD82|MD83|MD87|MD88|MD90/.test(all) || /TU134|TU154|TU204|TU214|C919|ARJ21|MC21/.test(all)) {
    return "Narrowbody";
  }
  if (/CRJ|CRJ1|CRJ2|CRJ7|CRJ9|CRJX|CRJ100|CRJ200|CRJ700|CRJ900|CRJ1000/.test(all) || /ERJ|E135|E140|E145|E170|E175|E190|E195|E290|E295/.test(all) || /AT42|AT43|AT45|AT46|AT72|AT75|AT76|ATR/.test(all) || /DH8|DHC8|Q200|Q300|Q400/.test(all) || /SF34|SF340|SB20|F50|F70|F100|DO228|DO328|RJ70|RJ85|RJ100/.test(all)) {
    return "Regional";
  }
  if (/GLEX|GLF|G280|G450|G500|G550|G650|G700/.test(all) || /CL30|CL35|CL60|GLOBAL|CHALLENGER/.test(all) || /FALCON|FA7X|FA8X|FA50|FA90|F2TH|F900|F2000/.test(all) || /CITATION|C25A|C25B|C25C|C56X|C68A|C700|LJ35|LJ45|LJ60|LJ75/.test(all) || /LEGACY|PRAETOR|PHENOM|E50P|E55P|HA420|PC24/.test(all)) {
    return "Business Jet";
  }
  ...
  if (/F14|F15|F16|F18|F22|F35|EUROFIGHTER|TYPHOON|RAFALE|GRIPEN|MIRAGE|SU27|SU30|SU35|MIG29|B1|B2|B52|TU95|TU160/.test(
    all
  )) {
    return "Military";
  }
...
async function maybeTriggerSpeech(adapter, base, a, speechText) {
  ...
  try {
    ...
    speechMode = String((st == null ? void 0 : st.val) || "browser");
  } catch {
    speechMode = "browser";
  }
...
function buildSpeechTextFromTemplate(a, display, template) {
  ...
  return String(template || "").replace(/\{([a-zA-Z0-9_]+)\}/g, (_match, key) => values[key] || "").replace(/\s+/g, " ").replace(/\s+\./g, ".").trim();
}
function cityOnly(name) {
  const v = (typeof name === "string" ? name : "").replace(/\bAirport\b/gi, "").replace(/\bInternational\b/gi, "").replace(/\bIntl\b/gi, "").replace(/\bFlughafen\b/gi, "").replace(/\bFort Worth\b/gi, "").replace(/\bMain\b/gi, "").replace(/\s+/g, " ").trim();
  if (v === "Frankfurt am") {
    return "Frankfurt";
  }
  return v;
}
function buildMessage(a, lang) {
  const lines = [];
  const routeCallsign = String(a.routeCallsign || a.callsign || "").trim().toUpperCase();
  lines.push("\u2708\uFE0F JetFrame");
  lines.push("");
  lines.push(`${a.icon || "\u2708\uFE0F"} ${modeText(a.mode || "", lang)}`);
  lines.push(`${(0, import_lang.t)(lang, "msgFlight")}: ${a.callsign || a.hex || "unknown"}`);
  if (routeCallsign) {
    lines.push(`${(0, import_lang.t)(lang, "msgRouteVia")}: ${routeCallsign}`);
  }
  if (a.airlineName) {
    lines.push(`${(0, import_lang.t)(lang, "msgAirline")}: ${a.airlineName}`);
  }
  if (a.routeTextLong) {
    lines.push(`${(0, import_lang.t)(lang, "msgRoute")}: ${a.routeTextLong}`);
  } else if (a.routeText) {
    lines.push(`${(0, import_lang.t)(lang, "msgRoute")}: ${a.routeText}`);
  } else if (a.routeWarning) {
    lines.push(`${(0, import_lang.t)(lang, "msgRoute")}: ${a.routeWarning}`);
  } else if (a.directionText) {
    lines.push(`${(0, import_lang.t)(lang, "msgDirection")}: ${a.directionText}`);
  }
  if (a.routeSource) {
    lines.push(`${(0, import_lang.t)(lang, "msgSource")}: ${a.routeSource}`);
  }
  if (a.aircraftModel) {
    lines.push(`${(0, import_lang.t)(lang, "msgAircraft")}: ${a.aircraftModel}`);
  } else if (a.aircraftType || a.type) {
    lines.push(`${(0, import_lang.t)(lang, "msgType")}: ${a.aircraftType || a.type}`);
  }
  if (a.registration) {
    lines.push(`${(0, import_lang.t)(lang, "msgReg")}: ${a.registration}`);
  }
  lines.push("");
  lines.push(`${(0, import_lang.t)(lang, "msgAltitude")}: ${Math.round(a.altFt || 0)} ft`);
  lines.push(`${(0, import_lang.t)(lang, "msgSpeed")}: ${Math.round(a.speedKt || 0)} kt`);
  lines.push(`${(0, import_lang.t)(lang, "msgClimbRate")}: ${Math.round(a.verticalRate || 0)} ft/min`);
  lines.push(`${(0, import_lang.t)(lang, "msgHeading")}: ${Math.round(a.trackDeg || 0)}\xB0`);
  const specialDisplayText = a.specialLiveryVisText || a.specialLiveryFull || a.specialText || "";
  if (specialDisplayText) {
    lines.push("");
    lines.push(
      `${a.isSpecial ? `\u2B50 ${(0, import_lang.t)(lang, "msgSpecial")}: ` : `\u2139\uFE0F ${(0, import_lang.t)(lang, "msgInfo")}: `}${specialDisplayText}`
    );
  }
  return lines.join("\n");
}
function modeText(mode, lang) {
  if (mode === "LANDING") {
    return (0, import_lang.t)(lang, "landing");
  }
  if (mode === "TAKEOFF") {
    return (0, import_lang.t)(lang, "takeoff");
  }
  if (mode === "OVERFLIGHT") {
    return (0, import_lang.t)(lang, "overflight");
  }
  return (0, import_lang.t)(lang, "flight");
}
async function ensureStates(adapter, config) {
  await ensureBaseStates(adapter, config);
}
async function keepExistingStringState(adapter, id, value) {
  ...
  try {
    const oldState = await adapter.getForeignStateAsync(id);
    return String((oldState == null ? void 0 : oldState.val) || "").trim();
  } catch {
    return "";
  }
}
