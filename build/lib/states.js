...
function cleanRouteCallsign(a) {
  const v = String(a.routeCallsign || "").trim().toUpperCase();
  const own = String(a.callsign || "").trim().toUpperCase();
  if (!v) {
    return "";
  }
  if (v === own) {
...
function trackDirectionText(degRaw) {
...
async function writeFlight(adapter, base, a) {
  const routeCallsign = String(a.routeCallsign || a.callsign || "").trim().toUpperCase();
  const display = await buildDisplayInfo(adapter, adapter.config, a);
...
function buildSpeechText(a, display) {
  const mode = String(a.mode || "").toUpperCase();
  ...
  const template = "{modeSpeechText}: {airlineName} {bestCallsign} {routeDirectionText} {routeOtherAirport} in {altitudeFt} Fu\xDF. {windowPositionSpeechText}.";
  const values = {
    modeSpeechText,
    routeDirectionText,
    routeOtherAirport,
  ...
  return template.replace(/\{([a-zA-Z0-9_]+)\}/g, (_match, key) => values[key] || "").replace(/\s+/g, " ").replace(/\s+\./g, ".").trim();
}
...
function cityOnly(name) {
  const v = String(name || "").replace(/\bAirport\b/gi, "").replace(/\bInternational\b/gi, "").replace(/\bIntl\b/gi, "").replace(/\bFlughafen\b/gi, "").replace(/\bFort Worth\b/gi, "").replace(/\bMain\b/gi, "").replace(/\s+/g, " ").trim();
  if (v === "Frankfurt am") {
    return "Frankfurt";
  }
  return v;
