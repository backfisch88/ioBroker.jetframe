...
async function updateAirportJson(adapter, logDebug, logWarn) {
  try {
    ...
    logDebug == null ? void 0 : logDebug(
      `Airport DB parsed: ${airports.length} Airports`,
      1
    );
    try {
      const deNames = await getGermanIataNamesCached(
        adapter,
        logDebug
      );
      ...
      logDebug == null ? void 0 : logDebug(
        `Airport DB DE-Namen erg\xE4nzt: ${Object.keys(deNames).length}`,
        1
      );
    } catch (e) {
      logWarn == null ? void 0 : logWarn(
        `Airport DB DE-Namen Fehler: ${(e == null ? void 0 : e.message) || e}`
      );
    }
    await adapter.setForeignStateAsync(
      `${adapter.namespace}.airportjson`,
    ...
    logDebug == null ? void 0 : logDebug(
      "Airport Datenbank aktualisiert",
      1
    );
  } catch (e) {
    logWarn == null ? void 0 : logWarn(
      `Airport DB Fehler: ${(e == null ? void 0 : e.message) || e}`
    );
  }
}
async function getGermanIataNamesCached(adapter, logDebug) {
  try {
    const st = await adapter.getForeignStateAsync(
      `${adapter.namespace}.airportjson`
    );
    ...
    if (raw && raw !== "[]") {
      ...
      if (Array.isArray(airports)) {
        ...
        if (Object.keys(cached).length > 100) {
          logDebug == null ? void 0 : logDebug(
            `Airport DB DE-Namen aus Cache \xFCbernommen: ${Object.keys(cached).length}`,
            1
          );
          return cached;
        }
...
function parseAirportCsv(csv) {
  ...
  for (let i = 1; i < lines.length; i++) {
    try {
      ...
      const lat = Number(
        row[idx("latitude_deg")]
      );
      const lon = Number(
        row[idx("longitude_deg")]
      );
      ...
      result.push({
        iata,
        icao: String(
          row[idx("icao_code")] || row[idx("gps_code")] || row[idx("ident")] || ""
        ).trim().toUpperCase(),
        name: String(
          row[idx("name")] || ""
        ).trim(),
        city: String(
          row[idx("municipality")] || ""
        ).trim(),
        country,
        flag: country,
        flagEmoji: countryFlagEmoji(
          country
        ),
        lat,
        lon,
    ...
    const req = https.get(
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (!text.includes("iata_code")) return reject(new Error("Airport CSV sieht ung\xFCltig aus: Header iata_code fehlt"));
          resolve(text);
        });
      }
    );
