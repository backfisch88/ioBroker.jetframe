...
function parseGermanIataPage(html) {
  ...
  for (const row of rows) {
    ...
    const iata = texts.find(
      (t) => /^[A-Z]{3}$/.test(t)
    );
    if (!iata) {
      continue;
    }
    const city = normalizeGermanCity(
      texts[3] || ""
    );
    if (city) {
      result[iata] = city;
...
function normalizeGermanCity(value) {
  return String(value || "").replace(/\s*\[[^\]]+\]/g, "").replace(/\s+/g, " ").replace(/^–$/, "").trim();
}
function decodeHtml(s) {
  return String(s || "").replace(/&amp;/g, "&").replace(/&nbsp;/g, " ").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&uuml;/g, "\xFC").replace(/&Uuml;/g, "\xDC").replace(/&ouml;/g, "\xF6").replace(/&Ouml;/g, "\xD6").replace(/&auml;/g, "\xE4").replace(/&Auml;/g, "\xC4").replace(/&szlig;/g, "\xDF");
}
