...
async function enrichFlightInfo(adapter, config, a, httpJson, httpText, logDebug, logWarn) {
  ...
  try {
    ...
    const baseInfo = {
      ...parsed,
      operationalCallsign,
      jetphotosUrl: parsed.registration ? "https://www.flightradar24.com/data/aircraft/" + encodeURIComponent(String(parsed.registration).toLowerCase()) : "",
...
function parseAdsbdbResponse(data, a, operationalCallsign, routeCallsign) {
  ...
  const logoUrl = logoKey ? `https://raw.githubusercontent.com/Jxck-S/airline-logos/refs/heads/main/radarbox_banners/${logoKey}.png` : "";
  const logoFallbackUrl = logoKey ? `https://raw.githubusercontent.com/Jxck-S/airline-logos/refs/heads/main/fr24_banners/${logoKey}.png` : "";
  return {
    operationalCallsign,
    routeCallsign,
    airlineName,
...
async function iataFromIcao(adapter, config, icao) {
  ...
  try {
    ...
    const found = airports.find(
      (a) => clean(a.icao || a.ICAO).toUpperCase() === icao
    );
    if (!found) return "";
    return clean(
      found.iata || found.IATA || ""
    ).toUpperCase();
  } catch {
    return "";
  }
...
async function cityNameFromIata(adapter, config, iata, logWarn) {
  ...
  try {
    ...
    const found = airports.find(
      (a) => clean(a.iata || a.IATA).toUpperCase() === code
    );
    ...
    if (airport) {
      return airport.replace(/international airport/ig, "").replace(/international/ig, "").replace(/airport/ig, "").replace(/\s+/g, " ").trim();
    }
    return code;
  } catch (e) {
...
async function getSystemLanguage(adapter) {
  ...
  try {
    ...
    return String(
      ((_a = obj == null ? void 0 : obj.common) == null ? void 0 : _a.language) || ((_b = obj == null ? void 0 : obj.native) == null ? void 0 : _b.language) || ""
    ).trim();
  } catch {
    return "";
  }
