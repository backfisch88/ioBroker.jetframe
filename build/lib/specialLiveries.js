...
async function updateSpecialLiveries(adapter, logDebug, logWarn) {
  try {
    ...
    logDebug == null ? void 0 : logDebug(
      `Special-Liveries HTML L\xE4nge: ${html.length}`,
      1
    );
    const liveries = parseSpecialLiveriesHtml(html);
    logDebug == null ? void 0 : logDebug(
      `Special-Liveries parsed: ${liveries.length}`,
      1
    );
    await adapter.setForeignStateAsync(
      `${adapter.namespace}.specialLiveries`,
      JSON.stringify(liveries),
      true
    );
    await adapter.setForeignStateAsync(
    ...
    logDebug == null ? void 0 : logDebug(
      `Special-Liveries DB aktualisiert: ${liveries.length} Eintr\xE4ge`,
      1
    );
  } catch (e) {
    logWarn == null ? void 0 : logWarn(
      `Special-Liveries DB Fehler: ${errorText(e)}`
    );
  }
}
...
function parseFromPlainText(text) {
  ...
  const regRe = /\b([A-Z0-9]{1,3}-[A-Z0-9]{3,5}|[A-Z]{1,2}\d{3,5}[A-Z]?|[A-Z]{2}-[A-Z0-9]{3}|N\d{1,5}[A-Z]{0,2}|JA\d{3,4}[A-Z]|B-\d{4}|C-[A-Z0-9]{4}|VH-[A-Z0-9]{3}|OE-[A-Z0-9]{3}|OO-[A-Z0-9]{3}|D-[A-Z0-9]{4})\b/g;
  let m;
  const hits = [];
...
function uniqueAndSort(items) {
  ...
  unique.sort(
    (a, b) => a.registration.localeCompare(b.registration)
  );
  return unique;
}
...
function htmlToText(html) {
  return decodeHtml(
    String(html || "").replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<br\s*\/?>/gi, " ").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim()
  );
}
function decodeHtml(s) {
  return String(s || "").replace(/&amp;/g, "&").replace(/&nbsp;/g, " ").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&uuml;/g, "\xFC").replace(/&ouml;/g, "\xF6").replace(/&auml;/g, "\xE4").replace(/&Uuml;/g, "\xDC").replace(/&Ouml;/g, "\xD6").replace(/&Auml;/g, "\xC4").replace(/&szlig;/g, "\xDF");
}
function clean(v) {
...
function guessTitle(description) {
  ...
  return d.replace(/\s+Livery$/i, "").replace(/\s+Special Livery$/i, "").trim();
}
...
function downloadText(url, redirects = 0) {
  return new Promise((resolve, reject) => {
      ...
      (res) => {
        if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          if (redirects >= 5) {
            reject(
              new Error(
                "Zu viele Redirects bei Special-Liveries"
              )
            );
            return;
          }
          const nextUrl = res.headers.location.startsWith("http") ? res.headers.location : new URL(
    ...
    req.on("timeout", () => {
      req.destroy(
        new Error(
          "Special-Liveries Download Timeout"
        )
      );
    });
    req.on("error", reject);
