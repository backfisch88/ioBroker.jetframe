...
const SPECIAL_LIVERIES_URL =
	'https://airportwebcams.net/special-liveries/';
...
export async function updateSpecialLiveries(
	...
	try {
		...
		logDebug?.(
	`Special-Liveries HTML Länge: ${html.length}`,
	1,
);
	...
	1,
		...
		logDebug?.(
			`Special-Liveries DB aktualisiert: ${liveries.length} Einträge`,
			1,
		);
	} catch (e) {
		logWarn?.(
...
export function parseSpecialLiveriesHtml(
	html: string,
): SpecialLiveryEntry[] {
...
function guessEmoji(description: string): string {
	...
	if (d.includes('retro')) return '🎨';
	if (d.includes('football') || d.includes('fanhansa') || d.includes('real madrid')) return '⚽';
	if (d.includes('pokemon') || d.includes('pikachu')) return '⚡';
	if (d.includes('star wars')) return '🤖';
	if (d.includes('anniversary') || d.includes('years') || d.includes('100')) return '🎉';
	if (d.includes('disney')) return '🏰';
	if (d.includes('sustainable') || d.includes('green')) return '🌱';
	if (d.includes('tintin')) return '🚀';
	if (d.includes('retro')) return '🛩️';
